...
static random_value mix_random_values (random_value r, random_value s) {
...
static bool random_bits (random_value *r, random_value s) {
...
/* #if _LIBC */
int try_tempname_len (char *, int, void *, int (*) (char *, void *), size_t);
/* #endif */

static int try_file (char *tmpl, void *flags) {
	int *openflags = flags;
	return __open (tmpl, (*openflags & ~O_ACCMODE) | O_RDWR | O_CREAT | O_EXCL, S_IRUSR | S_IWUSR);
}
...
int try_tempname_len (char *tmpl, int suffixlen, void *args, int (*tryfunc) (char *, void *), size_t x_suffix_len) {
	size_t len;
	char *XXXXXX;
	unsigned int count;
	int fd = -1;
...
#define ATTEMPTS_MIN (62 * 62 * 62)
	...
	random_value const biased_min = RANDOM_VALUE_MAX - RANDOM_VALUE_MAX % BASE_62_POWER;
...
int try_tempname (char *tmpl, int suffixlen, void *args, int (*tryfunc) (char *, void *)) {
